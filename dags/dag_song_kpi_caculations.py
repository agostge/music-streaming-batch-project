...
SONGS_FILE_PATH = 'raw/tracks.csv'
USERS_FILE_PATH = 'raw/users.csv'
STREAMS_FILE_PATH = 'raw/streams.csv'
...
def validate_datasets():
    ...
    try:
        stream_file = read_s3_csv(STREAMS_FILE_PATH)
        missing_columns = set(REQUIRED_COLUMNS['streams']) - set(stream_file.columns)
        if not missing_columns:
...
def upsert_to_postgres(df, table_name, id_columns):
    redshift_hook = PostgresHook(postgres_conn_id="streaming_db")
...
def calculate_genre_level_kpis():
    streams_data = read_s3_csv(STREAMS_FILE_PATH)
    songs_data = read_s3_csv(SONGS_FILE_PATH)
    ...
    upsert_to_postgres(final_kpis, 'genre_level_kpis', ['listen_date', 'track_genre'])
...
def calculate_hourly_kpis():
    ...
    upsert_to_postgres(final_kpis, 'hourly_kpis', ['listen_date', 'listen_hour'])
...
def move_processed_files():
    ...
    try:
        ...
        copy_source = {'Bucket': BUCKET_NAME, 'Key': STREAMS_FILE_PATH}
        destination_key = STREAMS_FILE_PATH.replace('raw/', 'archived/')
        s3.copy_object(CopySource=copy_source, Bucket=BUCKET_NAME, Key=destination_key)
        s3.delete_object(Bucket=BUCKET_NAME, Key=STREAMS_FILE_PATH)
        logging.info(f"Moved {STREAMS_FILE_PATH} to {destination_key}")
    except Exception as e:
        logging.error(f"Failed to move files from {STREAMS_FILE_PATH}: {str(e)}")
        raise
